...
def load_and_predict():
    ...
    try:
        ...
        print(f"Error: {REQUESTS_FILE} not found. Please run the sampling script first.")
        exit()

    # --- FIX 1: Standardize Patient ID ---
    ...
    # --- FIX 2: Ensure "Yes"/"No" Strings (Model Requirement) ---
    binary_cols = [
    ...
    for col in binary_cols:
        if col not in df.columns:
            ...
            # Convert any 1/0/True/False to String "Yes"/"No"
            df[col] = df[col].apply(lambda x: "Yes" if str(x).lower() in ['yes', 'true', '1'] else "No")

    # 2. Handle Contrast separately
...
def is_valid_slot(patient, scanner_name, current_dt, end_dt, scanner_stats):
    # Extract flags directly from dataset (String check is now safe)
    ...
    if hoist and scanner_stats[scanner_name]['Hoists'] >= MAX_HOISTS_PER_DAY:
        return False

    # Sedation: max 2 in a row
    ...
    if scanner_name == 'MRI Scanner 4' and current_dt.weekday() == 2 and start_hour >= 13:
        return False
    ...
    while queue and current_date <= END_DATE:
        ...
        for scanner_name in scanners:
            ...
            while curr < day_end and queue:
                ...
                if p.get('Hoist Required') == "Yes":
                    scanner_stats[scanner_name]['Hoists'] += 1

                if p.get('Sedation needed') == "Yes":
...
if __name__ == "__main__":
    ...
    # Check flags
    print("Input Data Flag Check:")
    print("Hoist Yes:", (df_new['Hoist Required'] == "Yes").sum())
    print("Class 1 Prediction Count:", (df_new['Predicted_Class'] == "Class 1").sum())

    schedule = run_scheduler(df_new)
    print("Appointments booked:", len(schedule))
